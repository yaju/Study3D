...
var ModelObject = (function (_super) {
    ...
    // モデルデータの設定
    ModelObject.prototype.setModelData = function () {
        ...
        for (var i = 0; i < this.isModels.length; i++) {
            var chkElm = this.isModels[i];
            if (chkElm.checked)
                models.push(modelClass[i]);
        }

        for (var m = 0; m < models.length; m++) {
            var modelData = models[m];
            ...
            for (var i = 0; i < modelData.faces.length; i += 3) {
                ...
                // 面列に新しい面を追加
                this.faces.push(new Face(this.vertices[v0 - 1 + addfaces], this.vertices[v1 - 1 + addfaces], this.vertices[v2 - 1 + addfaces]));
            }
